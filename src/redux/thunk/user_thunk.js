...
import { setGetCartList } from "../reducers/user_cart_reducer";
    ...
    try {
      ...
      if (result?.status === 200) {
        if (result.data.data.gauth) {
          ...
          dispatch(setUserGoogleLogin());
        } else if (result.data.message === "verification required") {
          setOTP(true);

          dispatch(setUserLoginOtp());
        } else {
...
export const user_mfa_thunk = (setShowQr, setQrValue) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(setUserEnableMFA(result.data));
    } catch (err) {
      console.log("err", err);
    }
  };
...
export const market_live_thunk = () => {
  return async (dispatch) => {
    dispatch(setMarketLive());
  };
};

export const market_live_off_thunk = () => {
  return async (dispatch) => {
    dispatch(setMarketLiveOff());
  };
};
...
export const get_cart_list_thunk = () => {
  return async (dispatch) => {
    try {
      ...
      dispatch(setGetCartList(result.data.data));
    } catch (err) {
      console.log(err);
    }
  };
