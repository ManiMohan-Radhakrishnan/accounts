...
const UPIWhitelist = () => {
  ...
  const upiVerify = async () => {
    ...
    try {
      ...
      if (result?.data?.status === 200) {
        ...
        dispatch(setUserUpiIdName(result?.data?.data?.name));
        dispatch(setUserUpiId(upi));
        if (result?.data?.data?.name) {
          setUpiIdValid(true);
          setInputDisabled(true);
          dispatch(setUserUpiIdValid(true));
          setShowVerify(true);
          setPanNameValid(false);
          dispatch(setUserPanNameValid(false));
          setPanMisMatchError("");
          dispatch(setPanMismatchErrorMsg(""));
          dispatch(setUserUpiIdDisabled(true));
        }
        if (result?.data?.data.name && result?.data?.data?.valid === false) {
          setPanName(result?.data?.data?.pan_name);
          dispatch(setUserPanName(result?.data?.data?.pan_name));
          setPanNameValid(true);
          dispatch(setUserPanNameValid(true));
          setInputDisabled(true);
          setUpiIdValid(false);
          dispatch(setUserUpiIdValid(false));
          setPanMisMatchError(result?.data?.message);
          dispatch(setPanMismatchErrorMsg(result?.data?.message));
        }
      }
  ...
  const handleReverify = () => {
    ...
    if (user?.panNameValid) {
      ...
      dispatch(setUserUpiId(upiList[0]?.payment_id));
    }
  };
            ...
            <>
              {kycStatus !== "success" || kycCountry !== "IN" ? (
                ...
                <button
                  ...
                  type="button"
                  onClick={() => {
                    dispatch(setWhitelistPopup(false));
                    setUpi(user?.upi_id);
                    setOpenVerifyModal(true);
                    setPanNameValid(user?.panNameValid);
                    setPanMisMatchError(user?.panMismatchError);
