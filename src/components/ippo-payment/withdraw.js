...
import { setWhitelistPopup } from "../../redux/reducers/user_reducer";

const IppoPaymentWithdraw = ({
  setWithdrawFund,
  withdrawFund,
    ...
    if (confirm.otp.length === 6) {
      if (confirm.password) {
        try {
          ...
          handleWithdrawProcess({
            payment_method: "ippopay",
            payment_slug: upiList[0]?.slug,
            amount: amount,
            address: null,
            network: null,
                  ...
                  {upiList.length === 0 || !upiList[0]?.upi_verified ? (
                    <div className={click && "pre-btn"}>
                      <p
                        ...
                        onClick={() => {
                          dispatch(setWhitelistPopup(true));
                          navigate("/accounts/whitelist");
                        }}
                      >
                        Please whitelist your UPI ID <FiArrowRight size={25} />
                      </p>
                    </div>
                  ) : (
                    <input
...
export default IppoPaymentWithdraw;
